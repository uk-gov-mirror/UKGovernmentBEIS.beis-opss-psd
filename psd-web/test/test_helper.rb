...
class ActiveSupport::TestCase
  ...
  def set_user_as_team_admin(user = User.current)
    allow(@keycloak_client_instance).to receive(:get_user_roles).with(user.id) { [:psd_user, :team_admin] }
  end
...
private
  ...
  def set_kc_user_as_opss(user_id)
    ...
    allow(@keycloak_client_instance).to receive(:get_user_roles).with(user_id).and_return([:opss_user])
  end
  ...
  def set_kc_user_as_non_opss(user_id)
    ...
    allow(@keycloak_client_instance).to receive(:get_user_roles).with(user_id).and_return([])
  end
